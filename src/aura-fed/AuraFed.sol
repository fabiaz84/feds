...
contract AuraFed is BalancerMetapoolAdapter{
            ...
            BalancerMetapoolAdapter(poolId_, dola_, vault_)
    {
        ...
        aura = IERC20(aura_);
        auraBal = IERC20(dolaBptRewardPool.stakingToken()); //TODO: Fix this - It's wrong
    ...
    */
    function contractAll() public {
        require(msg.sender == chair, "ONLY CHAIR");
        dolaBptRewardPool.withdraw(dolaBptRewardPool.balanceOf(address(this)), false, false);
    ...
    */
    function takeProfit(bool harvestLP) public {
        ...
        if(harvestLP && bptValue > dolaSupply) {
            ...
            require(dolaBptRewardPool.withdraw(bptToWithdraw, false, false), "AURA WITHDRAW FAILED");
        ...
        require(dolaBptRewardPool.getReward(address(this), true));
    ...
    /**
    @notice View function for getting crvLP tokens in the contract + convex dolaBptRewardPool
    */
